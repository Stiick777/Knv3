...
const handler = async (message, { conn, text, usedPrefix, command }) => {  
  ...
  // Respuesta mientras se descargan imágenes  
  ...
  try {  
    ...
    let results = [];
    let images = data.data.slice(0, 6); // Tomar hasta 6 imágenes

    for (let item of images) {  
      results.push({  
        body: proto.Message.InteractiveMessage.Body.fromObject({ text: null }),  
        footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: "⚡ KanBot ⚡" }),  
        header: proto.Message.InteractiveMessage.Header.fromObject({  
          title: `📍 ${item.grid_title || 'Imagen sin título'}`,  
